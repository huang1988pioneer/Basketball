...
- 連續命中連段：4 秒內延續命中可累積連段提示，投失或對手得分會中斷
...
也可以在 Godot 編輯器中開啟 `project.godot`，再執行主場景。瀏覽器版直接開啟 `index.html` 即可遊玩。

## 專案結構

- `scripts/game_data.gd`：版號、版面、模式、角色與技能常數
- `scripts/game_rules.gd`：投籃命中、抄球與速度計算
- `scripts/court_actor.gd`、`scripts/game_ball.gd`、`scripts/shot_flight.gd`：球場實體
- `scripts/main.gd`：遊戲迴圈、輸入與介面繪製
- `main.tscn`：主場景
...
- `tools/export_release.sh`：跨平台 release 輸出腳本
